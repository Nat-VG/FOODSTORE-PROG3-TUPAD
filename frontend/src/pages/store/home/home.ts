...
export async function renderStoreHome(): Promise<string> {
  ...
  let filtered = productos.filter((p) => p.disponible && !p.eliminado);
  if (filtroCategoria > 0) filtered = filtered.filter((p) => p.categoriaId === filtroCategoria);
  if (busqueda) filtered = filtered.filter((p) => p.nombre.toLowerCase().includes(busqueda.toLowerCase()));
  ...
  return `
    ...
    <main class="store-layout">
      ...
      <section class="catalog">
        ...
        <div class="product-grid">${grid || '<p class="empty">No hay productos disponibles.</p>'}</div>
      </section>
    </main>
    ${storeFooter()}`;
}
